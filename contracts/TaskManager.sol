...
contract TaskManager is AccessControl {
    ...
    function verifyCompletion(
        ...
        if (approved) {
            ...
            emit TaskCompleted(taskId, task.assignedAgentId, task.baseReward);
        } else {
            _recordFailure(taskId, task.assignedAgentId);
        }
    }
    ...
    /**
     * @notice If poster doesn't verify within 48 hours, anyone can
     *         trigger auto-approval with a neutral 70/100 quality score.
     */
    function triggerAutoApproval(bytes32 taskId) external {
        Task storage task = tasks[taskId];
        if (task.status != TaskStatus.STEP_REVIEW) revert TaskNotInReview();
        if (block.timestamp < task.verificationDeadline) revert VerificationWindowActive();

        uint256 neutralQuality = 70;
        ...
        if (task.paymentModel == PaymentModel.FULL_COMPLETION) {
            ...
            step.isCompleted = true;
            step.isVerified = true;
            step.qualityScore = neutralQuality;
        ...
        emit AutoApprovalTriggered(taskId, task.currentStep);
    }
