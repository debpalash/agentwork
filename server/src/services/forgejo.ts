/**
 * Forgejo Service — Programmatic git repository management for Collagent
 *
...
class ForgejoService {
  ...
  async createTaskRepo(taskId: string, title: string): Promise<any> {
    ...
    let repo: any;
    try {
      repo = await this.request(`/orgs/${config.orgName}/repos`, {
        method: "POST",
        body: JSON.stringify({
          name: repoName,
          description: `Collagent Task: ${title}`,
          private: true,
          auto_init: true,
          default_branch: "main",
          readme: "Default",
      ...
      repo = await this.request(`/repos/${config.orgName}/${repoName}`);
    }

    // Setup webhook for push events
    await this.setupWebhook(repoName).catch((err: any) => {
      if (!err.message.includes("409")) throw err;
    });
  ...
  async getCommits(taskId: string, limit = 20): Promise<any[]> {
    const repoName = this.taskIdToRepoName(taskId);
    return this.request<any[]>(`/repos/${config.orgName}/${repoName}/commits?limit=${limit}`);
  }

  /**
   * Get diff for a specific commit
   */
  async getCommitDiff(taskId: string, sha: string): Promise<any> {
    const repoName = this.taskIdToRepoName(taskId);
  ...
  async getDirectoryContents(taskId: string, path: string): Promise<any[]> {
    const repoName = this.taskIdToRepoName(taskId);
    return this.request<any[]>(`/repos/${config.orgName}/${repoName}/contents/${path}`);
  }
  ...
  async createAgentToken(username: string, taskId: string): Promise<string> {
    const token = await this.request<{ sha1: string }>(`/users/${username}/tokens`, {
      method: "POST",
      body: JSON.stringify({
